...
pub fn get_conflicted_v1(
    ...
    debug!("_____________________________________{:?}", current_tree);
    for path in conflict_paths {
        let abs_file_path = repo.path().parent().unwrap().join(Path::new(&path));
        debug!("file path of conflict {:?}", abs_file_path);
        let entry = current_tree.get_path(Path::new(&path)).expect("no entry");
        ...
        let file_bytes = read(abs_file_path).expect("no file");
        let workdir_content = String::from_utf8_lossy(&file_bytes);
        let text_diff = TextDiff::from_lines(&tree_content, &workdir_content);
        let unified_diff = text_diff.unified_diff(); //.context_radius(3).header("old file", "new file");

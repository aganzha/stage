...
use git2;
use std::sync::Once;
use crate::git::{make_diff, DiffKind, MARKER_OURS, Hunk};
...
pub fn choose_conflict_side() {
    ...
    // choose first conflict line in OUR side
    // "e.Description = any($2::text[])"
    let mut line = &hunk.lines[0];
    for l in &hunk.lines {
    ...
    debug!("{:?} offset {:?} ... {}", line.old_line_no, conflict_offset_inside_hunk, line.content);

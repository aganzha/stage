                    ...
                    if let Ok(commit) = repo.find_commit(oid) {
                        ...
                        match commit.parent_count() {
                            0 => {
                                ...
                                return Some((
                                    Some(commit),
                                    (left_commits.clone(), right_commits.clone()),
                                ));
                            }
                            1 => {
                                if let Ok(parent) = commit.parent_id(0) {
                                    ...
                                    if left_commits.contains_key(&parent)
                                        && right_commits.contains_key(&parent)
                                    {
                            ...
                            2 => {
                                if let Ok(left) = commit.parent_id(0) {
                                    left_commits
                                        .insert(left, commit.message().unwrap_or("").to_string());
                                }
                                if let Ok(right) = commit.parent_id(1) {
                                    right_commits
                                        .insert(right, commit.message().unwrap_or("").to_string());
                                }
